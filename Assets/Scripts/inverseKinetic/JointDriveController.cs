...
namespace Unity.MLAgentsExamples
{
    ...
    public class JointDriveController : MonoBehaviour
    {
        ...
        //BodyPart을 직접 받아서 설정하는 오버로드된 메서드, 이미 BodyPart 객체가 생성되어 있는 경우에 사용됩니다.
        public void SetupBodyPart(BodyPart bp)
        {
            bp.rb.maxAngularVelocity = k_MaxAngularVelocity;
            // Add & setup the ground contact script
            bp.groundContact = bp.rb.GetComponent<GroundContact>();
            if (!bp.groundContact)
            {
                bp.groundContact = bp.rb.gameObject.AddComponent<GroundContact>();
                bp.groundContact.agent = gameObject.GetComponent<Agent>();
            }
            else
            {
            ...
            bp.thisJdController = this;
            bodyPartsDict.Add(bp.rb.transform, bp);
            bodyPartsList.Add(bp);
        }
